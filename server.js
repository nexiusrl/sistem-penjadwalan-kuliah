...
function readDB() {
  try {
    ...
    return { users: [], dosen: [], ruangan: [], matakuliah: [], schedules: [], requests: [] };
  }
}
...
app.post('/api/auth/register', (req, res) => {
  ...
  if (role === 'dosen') {
    const dosenExists = db.dosen.some(d => d.name.toLowerCase() === name.toLowerCase());
    if (!dosenExists) {
      db.dosen.push({
...
app.put('/api/dosen/:id', (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ message: "Dosen tidak ditemukan." });
...
app.put('/api/ruangan/:id', (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ message: "Ruangan tidak ditemukan." });

  const oldName = db.ruangan[index].name;
  db.ruangan[index] = { ...db.ruangan[index], name, type, capacity: parseInt(capacity) };
...
app.put('/api/matakuliah/:id', (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ message: "Mata kuliah tidak ditemukan." });

  const oldName = db.matakuliah[index].name;
  db.matakuliah[index] = { ...db.matakuliah[index], name, code, sks: parseInt(sks) };
  
  // Sync subject name and code in schedules
  db.schedules = db.schedules.map(s => {
    if (s.subject === oldName) {
      return { ...s, subject: name, code: code };
    }
    return s;
  });
...
app.put('/api/schedules/:id', (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ message: "Jadwal tidak ditemukan." });
...
app.put('/api/requests/:id', (req, res) => {
  ...
  if (!reqItem) return res.status(404).json({ message: "Permohonan tidak ditemukan." });
